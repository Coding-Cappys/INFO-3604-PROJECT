...
class Session(db.Model):
    ...
    chair = db.relationship("User", foreign_keys=[chair_id], back_populates="chaired_sessions")
    ushers = db.relationship("User", secondary=session_ushers, back_populates="ushered_sessions")

    presentations = db.relationship("Presentation", back_populates="session")
    rsvps = db.relationship("RSVP", back_populates="session")
    attendances = db.relationship("Attendance", back_populates="session")
    feedbacks = db.relationship("Feedback", back_populates="session")
